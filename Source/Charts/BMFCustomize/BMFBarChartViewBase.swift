...
/*
 * 朱元斌2017-08-22添加 被用于BarChartView 继承 解决
 * 折线图和柱状图不同的操作
 */
import Foundation
import CoreGraphics
...
open class BMFBarChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    internal override func initialize()
    {
        ...
        self.addGestureRecognizer(_longPressGestureReconginzer)
        _doubleTapGestureRecognizer.isEnabled = _doubleTapToZoomEnabled
        _panGestureRecognizer.isEnabled = _dragEnabled
        
        #if !os(tvOS)
            _pinchGestureRecognizer = NSUIPinchGestureRecognizer(target: self, action: #selector(BMFBarChartViewBase.pinchGestureRecognized(_:)))
        ...
        else if recognizer.state == NSUIGestureRecognizerState.ended || recognizer.state == NSUIGestureRecognizerState.cancelled
        {
            ...
            if _isDragging
            {
                if recognizer.state == NSUIGestureRecognizerState.ended && isDragDecelerationEnabled
                {
                    ...
                    _decelerationDisplayLink = NSUIDisplayLink(target: self, selector: #selector(BMFBarChartViewBase.decelerationLoop))
